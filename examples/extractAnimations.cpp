...
int copyAnimations2(uGLTF::GLTFModel const & src, uGLTF::GLTFModel & dst )
{
    ...
    for(auto & a : src.animations)
    {
...

        {
            ...
            auto inputBufferViewIndex = newBuffer.createNewBufferView( inputCount * sizeof(float), 4);

            // Ouptut buffers, one for vec3 elements and one for vec4 elements.
            auto outputBufferViewIndexVec3 = newBuffer.createNewBufferView( vec3Count * sizeof(float) * 3, 4);
            auto outputBufferViewIndexVec4 = newBuffer.createNewBufferView( vec4Count * sizeof(float) * 4, 4);
